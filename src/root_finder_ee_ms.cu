...
namespace ccd
{
	...
	__device__ bool Origin_in_ee_inclusion_function_minimum_separation(const CCDdata &data_in, BoxCompute &box, CCDOut &out)
	{
		BoxPrimatives bp;
	...
	__device__ void edgeEdgeMinimumSeparationCCD(const CCDdata &data_in, const CCDConfig &config, CCDOut &out)
	{
		...
		while (!istack.empty())
		{
			...
			refine++;
			zero_in =
				Origin_in_ee_inclusion_function_minimum_separation(data_in, box, out);
