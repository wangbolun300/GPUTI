...
#pragma unroll
    ...
    __global__ void initialize_memory_pool(MP_unit *units, int query_size)
    {
        ...
        if (tx >= query_size)
        {
            units[tx].init(-1); //keep this to set box_in=true
        }
        else
        {
            units[tx].init(tx);
    ...
    __global__ void vf_ccd_memory_pool(MP_unit *units, int query_size, CCDdata *data, CCDConfig *config, int *results)
    {
        ...
        // {
            ...
            {                                       // if need check, and the for loop is not broken, do the check
                ...
                bool zero_in = Origin_in_vf_inclusion_function_memory_pool(data[box_id], units[qid]);
                if (zero_in)
                {
                    ...
                    widths[0] = units[qid].itv[0].second - units[qid].itv[0].first;
                    widths[1] = units[qid].itv[1].second - units[qid].itv[1].first;
                    widths[2] = units[qid].itv[2].second - units[qid].itv[2].first;
        ...
        // {
            if (data[box_id].sure_have_root > 0)
            {
                results[box_id] = 1; // get the results
            }
            // else
            // {
            //     results[box_id] = 0;
    ...
    __global__ void shift_queue_pointers(
       CCDConfig * config
    )
    {
        ...
        config[0].mp_remaining = config[0].mp_remaining < 0 ? config[0].mp_end + UNIT_SIZE - config[0].mp_start  : config[0].mp_remaining;
    }
                             ...
                             std::vector<bool> &result_list, int parallel_nbr, double &runtime)
    {
        ...
        // START HERE DAVID
        // create worker queue
        // int2 * d_queue; //we might change the datatype
        // unsigned SIZE = 2000000;
        // cudaMalloc((void**)&d_queue, sizeof(int2)*SIZE); 

        // // create the end of first loop
        // unsigned * d_end;
        // cudaMalloc((void**)&d_end, sizeof(unsigned));
        // cudaMemcpy(d_end, &nbr, sizeof(unsigned), cudaMemcpyHostToDevice);
        ...
        // int * d_nbr;
        // cudaMalloc((void**)&d_nbr, sizeof(int));
        // cudaMemcpy(d_nbr, &nbr, sizeof(int), cudaMemcpyHostToDevice);
        ...
        while (nbr_per_loop > 0)
        {
            ...
            cudaDeviceSynchronize();
            cudaMemcpy(&nbr_per_loop, &d_config[0].mp_remaining, sizeof(int), cudaMemcpyDeviceToHost);
        }
