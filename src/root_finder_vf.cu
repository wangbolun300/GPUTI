...
#pragma unroll
	...
	__device__ void split_dimension(const CCDOut &out,
									BoxCompute &box)
	...
	__device__ void bisect_vf_memory_pool(MP_unit &unit, int split,
										  const CCDConfig &config,
									   ...
									   int *results)
	{
		...
		bool zero_in = Origin_in_vf_inclusion_function_memory_pool(data_in, units_in);
		if (zero_in)
		{
			...
			// Condition 3, real tolerance is smaller than the input tolerance, return
			// true
			condition = units_in.true_tol <= config->co_domain_tolerance;
			if (condition)
			{
				results[box_id] = 1;
				return;
			}
			split_dimension_memory_pool(data_in, widths, split);
			MP_unit bisected[2];
			int valid_nbr;
			bisect_vf_memory_pool(units_in, split, config[0], bisected, valid_nbr);
